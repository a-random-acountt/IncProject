...
- **`Void`** (`Sources/Void`) — the SwiftUI app itself (views, theming,
  sound/haptics, persistence), built as an Xcode app target.
...
3. Xcode will resolve the three package dependencies
...
`XcodeProj` parser to confirm the target, its 22 source files, and the
asset catalog's Resources build phase all round-trip correctly - the
strongest check available without a Mac. Still, treat the very first
Xcode open as the real test.
